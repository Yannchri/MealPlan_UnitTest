...
public class MealPlanManagementUt
{
    ...
    public MealPlanManagementUt()
    {
        ...
        _mockMealPlanRepo.Setup(x => x.GetMealPlanById(1)).Returns(new MealPlan
        {
            Id = 1, Name = "Student Plan - Weekly - Valid", startDate = DateTime.Now.Subtract(TimeSpan.FromDays(1)),
            endDate = DateTime.Now.Add(TimeSpan.FromDays(5)), Price = 35
        });
        // Plan with id 2 is not active
        _mockMealPlanRepo.Setup(x => x.GetMealPlanById(2)).Returns(new MealPlan
        {
            Id = 2, Name = "Student Plan - Weekly - Invalid", startDate = DateTime.Now.Add(TimeSpan.FromDays(5)),
            endDate = DateTime.Now.Add(TimeSpan.FromDays(10)), Price = 35
        });
        // User with id 1 has enough credits
        _mockUserRepo.Setup(x => x.GetUserById(1)).Returns(new User
            { Id = 1, Name = "John Doe", Credits = 50, Email = "john.doe@test.ch" });
        // User with id 2 does not have enough credits
        _mockUserRepo.Setup(x => x.GetUserById(2)).Returns(new User
            { Id = 2, Name = "Jane Doe", Credits = 10, Email = "jane.doe@test.ch" });
        // User with id 3 is already subscribed to a plan
        _mockUserRepo.Setup(x => x.GetUserById(3)).Returns(new User
            { Id = 3, Name = "Jack Doe", Credits = 50, Email = "jack.doe@test.ch", MealPlanId = 1 });
    ...
    public void CreateMealPlan_ShouldAddMealPlanToRepository()
    {
        ...
        var newMealPlan = new MealPlan
            { Id = 3, Name = "New Plan", startDate = DateTime.Now, endDate = DateTime.Now.AddDays(7), Price = 50 };
    ...
    public void CreateMealPlan_ShouldNotAllowEmptyName()
    {
        // Arrange
        var newMealPlan = new MealPlan
            { Id = 3, Name = "", startDate = DateTime.Now, endDate = DateTime.Now.AddDays(7), Price = 50 };
    ...
    public void CreateMealPlan_ShouldNotAllowStartDateAfterEndDate()
    {
        // Arrange
        var newMealPlan = new MealPlan
            { Id = 3, Name = "New Plan", startDate = DateTime.Now.AddDays(7), endDate = DateTime.Now, Price = 50 };
    ...
    public void CreateMealPlan_ShouldNotAllowPriceLessOrEqualToZero()
    {
        // Arrange
        var newMealPlan = new MealPlan
            { Id = 3, Name = "New Plan", startDate = DateTime.Now, endDate = DateTime.Now.AddDays(7), Price = 0 };
